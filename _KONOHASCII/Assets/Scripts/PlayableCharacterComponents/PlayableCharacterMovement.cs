...
namespace PlayableCharacterComponents
{
    ...
    public class PlayableCharacterMovement : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("playerStates")] [Header("GENERAL MOVEMENT PRINCIPLES")]
        public EnumPlayerStates enumPlayerStates;
        ...
        private void MatchPlayerToWall()
        {
            ...
            bool HasAttemptedToAttachToWall() => !IsAttachedToWall() && wallgrabButton && HasDetectedWall();
            ...
            if (FetchGroundStatus() || !HasDetectedWall()) return;

            enumPlayerStates = EnumPlayerStates.Wall;
            ...
            SetRigidbodyScope(enumPlayerStates);
        ...
        {
            bool ShouldMoveOnGround() => !_isStandingOnWall;
        ...
        private void SetRigidbodyScope(EnumPlayerStates playerStates)
        {
            RigidbodyConstraints2D constraints;
            switch (playerStates)
            {
                case EnumPlayerStates.Free:
                    constraints = ~RigidbodyConstraints2D.FreezePosition | 
                                  RigidbodyConstraints2D.FreezeRotation;
                    break;
                case EnumPlayerStates.Stagger:
                case EnumPlayerStates.Busy:
                    constraints = RigidbodyConstraints2D.FreezeAll;
                    break;
                case EnumPlayerStates.Wall:
        ...
        {
            float ButtonInputHelper(InputAction.CallbackContext helperContext) 
                => helperContext.ReadValue<Vector2>().x;
            xMovementAxisInput = ButtonInputHelper(context);
        }

        public void InputJump(InputAction.CallbackContext context)
        {
            void ButtonInputHelper (InputAction.CallbackContext helperContext)
            {
                if (helperContext.started)
                {
                    isHoldingJump = true;
                    hasJumped = true;
                }
                if (helperContext.canceled) isHoldingJump = false;
            }
        ...
        {
            bool ButtonInputHelper(InputAction.CallbackContext helperContext)
                => helperContext.performed && !helperContext.canceled;

            wallgrabButton = ButtonInputHelper(context);
        }
